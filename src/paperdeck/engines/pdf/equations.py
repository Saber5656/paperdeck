...
def process_equations(
    seg: Any, blocks: list[Any], pdfdoc: Any, llm: Any, ledger: Any, alloc: Any
) -> EquationsResult:
    ...
    for block_id in seg.order:
        ...
        try:
                    ...
                    if rest and rest.role == "display_equation":
                        ...
                        try:
                            ...
                            result.equations[rest_id] = EquationDraft(
                                ra, _number(rest.number_text, sequence), rid
                            )
                        except Exception:
            ...
            messages = [
                {
                    "role": "system",
                    "content": load_prompt(
                        "equation_latex", context="The image is the source of truth."
                    ),
                },
                {"role": "user", "content": "Transcribe this equation image."},
            ]
            ...
            if (
                not latex
                or len(latex) > 4000
                or re.search(r"\\(?:input|include|write|csname|href|url)\b", latex)
            ):
                result.warnings.append(f"vlm-latex-rejected:{block_id}")
            else:
                draft = EquationDraft(
                    anchor_id, number, asset_id, latex=latex, confidence=float(response.confidence)
                )
        except Exception:
